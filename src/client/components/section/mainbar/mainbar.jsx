                ...
                <div className='flex flex-col flex-1 w-full gap-3 p-3' >
                    {
                        messages && messages.map((item) => (
                                <div key={item._id} className={`${item.question ? 'ml-auto' : 'mr-auto'}  max-w-[80%] flex flex-col gap-3`}>
                                    {
                                        item.question &&
                                    ...
                                    }
                                    {
                                        ...
                                        <Card className='w-[235px]'>
                                            <CardBody className={`p-3 relative flex gap-2 ${item.attachments.length >= 1 && 'flex-row'} ${item.attachments.length >= 2 && 'flex-wrap'} `}>
                                                {
                                                    (item.attachments.length > 2 || item.attachments.length <= 4) && item.attachments.map((image, idx) => {
                                                        if (idx >= 4) return
                                                        return (
                                                            <div key={idx} className='blur-[2px]'>
                                                                <Image
                                                                    width={100}
